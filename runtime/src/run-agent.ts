...
function cmdOutput(result: { stdout: string; stderr: string }): string {
  const msg = (result.stderr || result.stdout || "").trim();
  return msg || "No command output.";
}

function findBranchWorktree(repoDir: string, branch: string): string | null {
...
function setupWorktree(repoDir: string, worktreesDir: string, worktreePath: string, branch: string): string {
  ...
  if (fs.existsSync(worktreePath) && fs.statSync(worktreePath).isDirectory()) {
    ...
    if (pullResult.returncode !== 0) {
      console.log(
        `Warning: git pull failed in existing worktree (${branch}). Continuing.\n${cmdOutput(pullResult)}`,
      );
    }
...
function copyEnv(repoDir: string, worktreePath: string): void {
  if (!(fs.existsSync(worktreePath) && fs.statSync(worktreePath).isDirectory())) {
...
function tailFile(filePath: string, maxLines = 25): string {
...
function findExistingPr(ghRepo: string, branch: string): [number | null, string | null] {
  const result = _run([
    "gh",
    "pr",
    "list",
...
function runOrThrow(cmd: string[], cwd?: string, timeout = 120): string {
...
function scaffoldNextJs(repoDir: string, repoName: string): void {
  const packageJson = {
    name: repoName,
    version: "0.1.0",
    private: true,
  ...
  writeFileIfMissing(
    path.join(repoDir, "next-env.d.ts"),
...
function scaffoldRepoTemplate(repoDir: string, template: RepoTemplate, repoName: string): void {
  switch (template) {
    case "nextjs":
...
function commitAndPushIfDirty(repoDir: string, message: string): void {
...
function defaultTemplateOptions(template: RepoTemplate): { ciCmd: string; installCmd: string; techStack: string } {
  switch (template) {
    case "nextjs":
      return {
        ciCmd: "npm run build",
...
function resolveRepoSlug(repoKey: string, owner: string): { repoName: string; ghRepo: string } {
  if (repoKey.includes("/")) {
...
): ReturnType<typeof loadRepoConfig> {
...
function loadSessionDeliveries(swarmDir: string): SessionDelivery[] {
  const candidates = [
  ...
  for (const stateDir of candidates) {
    ...
    try {
      const raw = JSON.parse(fs.readFileSync(sessionFile, "utf8")) as Record<string, Record<string, unknown>>;
      const rows: SessionDelivery[] = [];
...
function detectOriginNotifyContext(swarmDir: string): { channel: string; target: string; replyTo: string } | null {
...
function resolveTaskNotifyContext(
  swarmDir: string,
  providedChannel: string,
  providedTarget: string,
  providedReplyTo: string,
...
function parseCliOptions(extraArgs: string[]): SpawnCliOptions {
...
export function spawnAgent(args: SpawnAgentArgs): void {
  ...
  const config =
    loadRepoConfig(reposFile, repoKey) ??
  ...
  for (const task of store.listRunning()) {
    if (
      ...
      console.log(
        `Branch '${branch}' already has an active agent task '${task.id}' (PID ${task.pid}). Stop or reuse that task first.`,
      );
      process.exit(1);
    }
  }
  ...
  try {
    ...
    if (installResult.returncode !== 0) {
      ...
      throw new Error(
        `Dependency install failed (exit ${installResult.returncode}) for '${installCmd}'.\n${installErr || "No command output available."}`,
      );
    }
