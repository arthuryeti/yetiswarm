...
function runtimeDefaults(): { dbPath: string; reposFile: string } {
...
function consumeDbPath(args: string[]): [string, string[]] {
...
function consumeReposFile(args: string[]): [string, string[]] {
  const defaults = runtimeDefaults();
  if (!args.length) {
...
function cmdListRunning(args: string[]): void {
...
function cmdCountRunning(args: string[]): void {
  const [dbPath] = consumeDbPath(args);
  const store = new TaskStore(dbPath);
  console.log(store.countRunning());
}

function cmdGetField(args: string[]): void {
...
function cmdGetFields(args: string[]): void {
  const obj = JSON.parse(args[0] || "{}");
...
function cmdParsePr(args: string[]): void {
...
function cmdRepoConfig(args: string[]): void {
  ...
  console.log(
    `${config.path}\t${config.ghRepo}\t${config.worktrees}\t${config.ciCmd}\t${config.installCmd}`,
  );
}

function cmdRepoKeys(args: string[]): void {
  const [reposFile] = consumeReposFile(args);
  console.log(listRepoKeys(reposFile).join(", "));
}

function cmdFormatPrFeedback(args: string[]): void {
...
function cmdDumpTasks(args: string[]): void {
  const [dbPath] = consumeDbPath(args);
  const store = new TaskStore(dbPath);
...
function cmdDumpEvents(args: string[]): void {
  const [dbPath, rest] = consumeDbPath(args);
