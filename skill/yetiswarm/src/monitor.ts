...
const enum TaskState {
  ...
  AGENT_DEAD_NO_PR = "AGENT_DEAD_NO_PR",
  DETECT_PR = "DETECT_PR",
...
interface MonitorContext {
  ...
  defaultNotifyReplyTo: string;
  progressTarget: string;
  progressReplyTo: string;
  progressCooldownSecs: number;
  ghBin: string;
}
...
interface Logger {
  ...
  exception: (prefix: string, err: unknown) => void;
}
...
function setupLogging(logsDir: string): Logger {
...
function tryAcquireLock(lockPath: string): MonitorLock | null {
  try {
...
function acquireMonitorLock(swarmDir: string): MonitorLock | null {
  const lockPath = path.join(swarmDir, ".monitor.lock");
  return tryAcquireLock(lockPath);
}

function releaseMonitorLock(lock: MonitorLock): void {
  try {
...
function ghPrDetails(ctx: MonitorContext, prNumber: number, ghRepo: string): PrDetails {
  const result = _run([
    ctx.ghBin,
    "pr",
    "view",
...
function ghPrList(ctx: MonitorContext, ghRepo: string, branch: string): string {
  const result = _run([
    ctx.ghBin,
    "pr",
    "list",
...
function ghPrChecks(ctx: MonitorContext, prNumber: number, ghRepo: string): string {
  const result = _run([
    ctx.ghBin,
    "pr",
    "checks",
...
function ghPrReviews(ctx: MonitorContext, prNumber: number, ghRepo: string): string {
  const result = _run([
    ctx.ghBin,
    "pr",
    "view",
...
function ghApi(ctx: MonitorContext, endpoint: string): string {
...
function resolveNotifyConfig(
  ctx: MonitorContext,
  task: Task,
): { channel: string; target: string; replyTo: string } {
...
function sendMessage(channel: string, target: string, replyTo: string, message: string): void {
  if (!target) {
    return;
  }
  const cmd = [
...
function notifyTask(ctx: MonitorContext, task: Task, message: string): void {
  const cfg = resolveNotifyConfig(ctx, task);
  sendMessage(cfg.channel, cfg.target, cfg.replyTo, message);
}

function taskRepoLabel(task: Task): string {
  return `${task.gh_repo || task.repo_key}`;
}

function buildPrMessage(
  action: string,
  task: Task,
  detail: {
    title?: string;
...
function checkFlag(task: Task, key: string): boolean {
  return Boolean((task.checks as Record<string, unknown> | undefined)?.[key]);
}

function markFlag(ctx: MonitorContext, task: Task, key: string): void {
  ctx.store.patchTask(task.id, { checks: { [key]: true } });
  task.checks = { ...(task.checks ?? {}), [key]: true };
}

function isSessionIdle(ctx: MonitorContext, taskId: string): boolean {
...
function respawnTask(ctx: MonitorContext, task: Task, errorContext = ""): void {
...
function respawnForReview(ctx: MonitorContext, task: Task, commentRetries: number, feedbackText: string): void {
  ...
  log.info(
    "  Respawning %s for review fixes (attempt %d/%d)",
    task.id,
    newCommentRetries,
    ctx.maxCommentRetries,
  );
...
function determineState(task: Task): TaskState {
...
function handleAwaitingMerge(task: Task, ctx: MonitorContext): TaskState | null {
...
function handleCheckAgent(task: Task, ctx: MonitorContext): TaskState | null {
...
function handleAgentDeadNoPr(task: Task, ctx: MonitorContext): TaskState | null {
  if (task.retries < ctx.maxRetries) {
    log.info("  Session dead, no PR — respawning");
    notifyTask(
      ctx,
      task,
      `⚠️ \`${task.id}\` died before creating a PR. Respawning (attempt ${task.retries + 1}/${ctx.maxRetries})...`,
    );
...
function handleDetectPr(task: Task, ctx: MonitorContext): TaskState | null {
...
function handleCheckMerge(task: Task, ctx: MonitorContext): TaskState | null {
...
function ghPrMergeable(ctx: MonitorContext, prNumber: number, ghRepo: string): string {
  const result = _run([
    ctx.ghBin,
    "pr",
    "view",
...
function handleConflicts(task: Task, ctx: MonitorContext): TaskState | null {
  ...
  if (commentRetries >= maxConflictRetries) {
    ...
    return TaskState.CHECK_CI; // skip to CI, let normal flow continue
  }
  ...
  notifyTask(
    ctx,
    task,
    `🔀 \`${task.id}\` has merge conflicts — auto-resolving (attempt ${commentRetries + 1}/${maxConflictRetries})...`,
  );
...
function handleCheckCi(task: Task, ctx: MonitorContext): TaskState | null {
  ...
  if (ciStatus === "fail") {
    ...
    if (task.retries < ctx.maxRetries) {
      notifyTask(
        ctx,
        task,
        `🔁 \`${task.id}\` CI failed — auto-fixing (attempt ${task.retries + 1}/${ctx.maxRetries})...`,
      );
...
function handleCheckReviews(task: Task, ctx: MonitorContext): TaskState | null {
    ...
    if (commentRetries < ctx.maxCommentRetries) {
      ...
      if (feedback) {
        ...
        notifyTask(
          ctx,
          task,
          `🔧 \`${task.id}\` — auto-fixing review feedback from ${reviewers} (attempt ${newCommentRetries}/${ctx.maxCommentRetries})...\nPR: ${prUrl}`,
        );
        ...
        notifyTask(
          ctx,
          task,
          buildPrMessage("Review Requested", task, {
...
function handleAllGreen(task: Task, ctx: MonitorContext): TaskState | null {
...
function cleanupMergedTask(task: Task, ctx: MonitorContext, prDetails?: PrDetails): TaskState | null {
...
const HANDLERS: Record<TaskState, (task: Task, ctx: MonitorContext) => TaskState | null> = {
  [TaskState.AWAITING_MERGE]: handleAwaitingMerge,
  [TaskState.CHECK_AGENT]: handleCheckAgent,
  [TaskState.AGENT_DEAD_NO_PR]: handleAgentDeadNoPr,
  [TaskState.DETECT_PR]: handleDetectPr,
  [TaskState.CHECK_MERGE]: handleCheckMerge,
  [TaskState.CHECK_CI]: handleCheckCi,
  [TaskState.CHECK_REVIEWS]: handleCheckReviews,
  [TaskState.ALL_GREEN]: handleAllGreen,
};

function maybeSendProgressDashboard(ctx: MonitorContext, tasks: Task[]): void {
...
function processTask(task: Task, ctx: MonitorContext): void {
  ...
  let state: TaskState | null = determineState(task);
  while (state) {
