...
import { _run, setupCronEnv, killAgent, gitWorktreeRemove, gitWorktreePrune, gitBranchDelete, gitRemoteBranchDelete, gitRemotePrune, } from "./shell.js";
import { TaskStore, listRepoKeys, loadRepoConfig } from "./task-store.js";
import { resolveRuntimePaths } from "./runtime-paths.js";
function ghPrState(ghBin, ghRepo, prNumber) {
    if (!ghRepo || !prNumber) {
        return "";
    }
    const result = _run([
...
function branchHasMergedPr(ghBin, ghRepo, branch) {
    if (!ghRepo || !branch) {
        return false;
    }
    const result = _run([
...
function listLocalBranches(repoDir) {
    const result = _run([
        "/usr/bin/git",
        "-C",
        repoDir,
...
export function cleanup(dbPath, reposFile) {
